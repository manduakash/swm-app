...
import { getWasteCollectionData } from "@/app/commonAPIs"; // Update API function name accordingly
...
export function GetWasteNonSegregated() {
    ...
    const fetchPropertiesData = async () => {
        try {
            const data = await getWasteCollectionData();
                    ...
                    {loading ? (
                        ...
                        currentRecords?.map((property, index) => (
                            <TableRow key={index} className="cursor-pointer hover:bg-gray-100 transition">
                                <TableCell>{property?.UniqueNumber ?? "N/A"}</TableCell>
                                ...
                                <TableCell>{property?.CollectionType ?? "N/A"}</TableCell>
                                <TableCell>
                                    <Dialog>
